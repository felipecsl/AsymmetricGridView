...
public class SampleDbAdapter {
    ...
    public Cursor fetchDataByName(String inputText) throws SQLException {
        ...
        if (inputText == null || inputText.length() == 0) {
            mCursor = mDb.query(SQLITE_TABLE, new String[]{KEY_ROWID, KEY_TEXT, KEY_ROW_SPAN,
                            KEY_COL_SPAN},
                    null, null, null, null, null);

        } else {
            mCursor = mDb.query(true, SQLITE_TABLE, new String[]{KEY_ROWID, KEY_TEXT, KEY_ROW_SPAN,
                            KEY_COL_SPAN},
                    KEY_TEXT + " like '%" + inputText + "%'", null,
                    null, null, null, null);
        }
        if (mCursor != null) {
            mCursor.moveToFirst();
    ...
    public Cursor fetchAllData() {
        Cursor mCursor = mDb.query(SQLITE_TABLE, new String[]{KEY_ROWID, KEY_TEXT, KEY_ROW_SPAN,
                        KEY_COL_SPAN},
                null, null, null, null, null);
